...
elif [ "$model" = "lattice" ]; then
    CUBLAS_WORKSPACE_CONFIG=:4096:8 python run_multimodal.py --dataset ${dataset} --model lattice --method zeros > lattice_${dataset}_zeros.out 2>&1
    CUBLAS_WORKSPACE_CONFIG=:4096:8 python run_multimodal.py --dataset ${dataset} --model lattice --method random > lattice_${dataset}_random.out 2>&1
    CUBLAS_WORKSPACE_CONFIG=:4096:8 python run_multimodal.py --dataset ${dataset} --model lattice --method mean > lattice_${dataset}_mean.out 2>&1
