...
def main():
    ...
    if not os.path.exists(logs_path + f'/{args.dataset}/pers_page_rank/{args.model}/'):
        os.makedirs(logs_path + f'/{args.dataset}/pers_page_rank/{args.model}/')

    if not os.path.exists(scripts_path + f'/{args.dataset}/pers_page_rank/{args.model}/'):
        os.makedirs(scripts_path + f'/{args.dataset}/pers_page_rank/{args.model}/')
    ...
    for hyperparam in hyperparams:
        ...
        completed = False
        if os.path.isfile(f'{logs_path}/{args.dataset}/pers_page_rank/{args.model}/{logfile}'):
            with open(f'{logs_path}/{args.dataset}/pers_page_rank/{args.model}/{logfile}', 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
...
"""
    ...
    if header:
        for index, offset in enumerate(range(0, nb_jobs, args.batch_size_jobs), 1):
            ...
            with open(scripts_path + f'/{args.dataset}/pers_page_rank/{args.model}/' + date_time + f'__{index}.sh', 'w') as f:
                if args.cluster == 'cineca':
